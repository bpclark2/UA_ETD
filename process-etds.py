...
with open('S:/Metadata/ETD/content/dc-etds.csv', 'w', encoding='utf-8') as csv_file:
    ...
    for subdir in os.listdir(start_dir):
        files = [f for f in os.listdir(f'{start_dir}/{subdir}')]
        for i in files:
            if '_DATA.xml' in i:
                with open(f'{start_dir}/{subdir}/{i}', 'r', encoding='utf-8') as f:
                    tree = ET.parse(f)
                    record = tree.getroot()
                    ...
                    degree = record.find('.//DISS_degree').text
                    degree_type = degree_dict[degree]
                    dept = record.find('.//DISS_inst_contact').text
                    dept_heading = dept_dict[dept]
                    ...
                    diss_date = record.find('.//DISS_comp_date').text.strip().split('-')[0]
                    ...
                    discipline = record.find('.//DISS_cat_desc').text.replace('&amp;', '&')
